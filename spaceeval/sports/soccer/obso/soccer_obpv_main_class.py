...
class SoccerObpv(AbstractSpaceModel):
    ...
    def __init__(
        ...
        super().__init__(
            event_data_path, tracking_home_path, tracking_away_path, out_path, testing_mode
        )
    ...
    def calculate(self) -> Dict[str, pd.DataFrame]:
        ...
        match_ids = self.event_data.keys()
        ...
        for match_id in iterator:
            ...
            try:
                results[match_id] = calculate_obpv(
                    self.event_data[match_id],
                    self.tracking_home_data[match_id],
                    self.tracking_away_data[match_id],
                    self.testing_mode
                )
